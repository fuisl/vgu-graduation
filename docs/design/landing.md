...
4. **Parallax** (`BackgroundMotion.tsx`) — a side-effect-only client component that writes `--scroll-y`, `--pointer-x`, `--pointer-y` onto the document root (rAF-throttled). Only the aurora consumes these via CSS `calc()` in its `transform`, eased through a 0.5s `ease-out` transition: it shifts opposite scroll at -0.05x and nudges toward the cursor at 16px. The grid stays still. Cursor tracking only attaches on `(hover: hover) and (pointer: fine)` devices (no touch jitter), and the whole component no-ops under `prefers-reduced-motion: reduce`.
