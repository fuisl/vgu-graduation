...
- Visible landing copy uses the shared ASCIIGen-style `DecodeText` primitive on first arrival. The headline begins the stagger, followed by navigation, year, event metadata, message, subtitle and credit; each element decodes exactly once per page mount. "Coming soon in November." joins that entrance at 780ms and remains the only line that repeats, holding for 4.2s between later runs. Unresolved text flickers and locks left-to-right using a pool balanced across three engine families—ASCII punctuation, blocks/quadrants, and Braille. Noise updates are capped near 30fps, deterministic within each frame, and rendered in Geist Mono while invisible original glyphs reserve the final text width, preventing layout shifts. Words wrap together on narrow screens. Accessible copies remain constant; hidden pages cancel active work, and reduced motion shows plain text.
