...
3. Leave the Install Command on **Automatic**. The committed `pnpm-lock.yaml` and root `packageManager` select pnpm 10. A manual `pnpm install` override may select an older pnpm version on Vercel.
